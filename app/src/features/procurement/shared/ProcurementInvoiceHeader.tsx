...
export function ProcurementInvoiceHeader({ order, setReload, role }: {
    ...
    async function updateStatus() {
        try {
            setProcess(true);
            const status = order.status === 'SENT' ? 'CONFIRMED' : order.status === 'CONFIRMED' ? 'DELIVERED' : "RECEIVED";
        ...
        <>
                ...
                <div className="flex-1 bg-slate-50 p-4 rounded-md shadow-sm">
                    ...
                    <div className="flex-center-y gap-2 text-sm text-gray-500 uppercase font-semibold my-2">
                            ...
                            order.status === 'DELIVERED' ? (
                                <>
                                    ...
                                    <button onClick={() => setReceived(true)}>
                                        <ProcurementBadge label="RECEIVED" className="!text-sm grayscale" />
                                    </button>
                                </>
                            ) : (
                                <ProcurementBadge label={order.status} className="!text-sm" />
                        ...
                        {role === 'SUPPLIER' && (
                            <>
                                <ProcurementBadge label={order.status} className="!text-sm" />
                                <button onClick={() => setOpen(!open)} className="ms-auto">
            ...
            <div className="flex-center my-8 drop-shadow-sm">
                ...
                <div className="relative">
                    <ProcurementBadge 
                        label="DELIVERED" 
                        className={`absolute -top-5 text-center left-1/2 -translate-x-1/2 text-2xs ${!order.delivered_date && "grayscale"}`} 
                    />
                    <div className="absolute -bottom-5 text-2xs text-center left-1/2 -translate-x-1/2 w-10 font-bold uppercase">
                        { order.delivered_date && shortDate(formatDateTime(order.delivered_date)) }
                    </div>
                ...
                <Separator className={`${order.received_date ? timelineReached : timelineUnreached} py-1 !w-18`} />
                <div className="relative">
                    <ProcurementBadge 
                        label="RECEIVED" 
                        className={`absolute -top-5 text-center left-1/2 -translate-x-1/2 text-2xs ${!order.received_date && "grayscale"}`} 
                    />
                    <div className="absolute -bottom-5 text-2xs text-center left-1/2 -translate-x-1/2 w-10 font-bold uppercase">
                        { order.received_date && shortDate(formatDateTime(order.received_date)) }
                    </div>
                    <div className={`${order.received_date ? timelineReached : timelineUnreached} flex-center rounded-full w-8 h-8 border-1 border-white`}>
                        <Truck className="w-4 h-4 text-white" />
                    </div>
                </div>
            </div>
            ...
            <AlertDialog open={open} onOpenChange={setOpen}>
                <AlertDialogContent className="w-fit flex flex-col gap-6">
                    <AlertDialogTitle>
                        <ProcurementHeader label="Update order status?" />
                    </AlertDialogTitle>
                    ...
                    <div className="text-center font-semibold uppercase text-gray-500 text-sm">
                        <span className="text-black">WARNING:</span> this action is irreversible.
                    </div>
            ...
            <AlertDialog open={ received } onOpenChange={ setReceived }>
                <AlertDialogContent className="w-fit flex flex-col gap-6">
                    <AlertDialogTitle><ProcurementHeader label="Purchase order received??" /></AlertDialogTitle>
                    <div className="mx-auto">
                        <PackageCheck className="h-40 w-30 text-green-900" />
                    </div>
