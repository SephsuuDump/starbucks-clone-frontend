...
export function InvoicePreview({ activeInvoice }: Props) {
    if (!activeInvoice) return <div>No purchase orders yet</div>
    return(
        <section className="flex flex-col gap-2 col-span-2 p-3 bg-white rounded-md shadow-sm">
            ...
            <div className="grid grid-cols-2">
                ...
                {activeInvoice.supplies.map((item: any, _: number) => (
                    <Fragment key={_}>
                        <div className="border-b-1 text-sm p-1 px-2 border-r-1">{item.name}</div>
