    ...
    return(
        <Dialog open onOpenChange={ (open) => { if (!open) setRequest(undefined) } }>
            <DialogContent>
                ...
                <div className="flex justify-between text-sm -mt-2">
                    <div>Status</div>
                    <ProcurementBadge label={ activeRequest.status } style="bg-yellow-100 text-yellow-600" />
                </div>
                <Separator className="-mt-2" />
                ...
                {activeRequest.purchase_order_item.map((item: any, _: number) => (
                    <div className="tdata grid grid-cols-3 !-mt-4" key={_}>
                        <div className="td">{ item.supplier_item.name }</div>
                        <div className="td">{ item.quantity }</div>
                        <div className="td">{ formatToPeso(item.supplier_item.unit_cost) }</div>
                    </div>
                ))}
                <ModalButton
                    onClick={ () => updateStatus(activeRequest.id, "PENDING") }
                    icon={ CircleCheckBig }
                    onProcess={ onProcess }
                    label="Approve Request"
                    loadingLabel="Processing Request"
                    style="!bg-green-900 hover:opacity-90"
                />
