...
import { Box, CheckCircle, ScanEye } from "lucide-react";
...
export default function ListTransfer() {
    ...
    async function handleApproved(id : string ) {
        ...
        try {
            const data = await TransferService.updateStatus(id, 'OUT');   
            if(data) {
    ...
    return (
        <>
            ...
            <div className="flex flex-col bg-white shadow-sm rounded-xl w-full p-6 mt-5">
                ...
                <div className="flex rounded-lg shadow-sm p-4 mb-3 border-gray-200 justify-center font-light text-sm bg-gray-200">
                    No data found 
                </div> : 
                <>
                    ...
                    >
                        <div className="grid grid-cols-4 items-center text-sm font-light text-gray-800">
                            ...
                            <div className="flex justify-center gap-2">
                                ...
                                <Button
                                    className="!bg-green-700 text-white px-2 py-1 rounded-md text-sm hover:opacity-90"
                                    onClick={() => {
                                        handleViewing(request.id)
                                        setConfirmOpen(true)
                                    }}
            ...
            <div className="flex flex-col bg-white shadow-sm rounded-xl w-full p-6 mt-5">
                ...
                <div className="grid grid-cols-5 text-sm font-semibold text-gray-700 mt-5 mb-2 items-center justify-center">
                    <div className="flex justify-center">TRANSFER ID</div>
                    <div className="flex justify-center">FROM</div>
                    <div className="flex justify-center">TO</div>
                ...
                {transfers.OUT.length === 0 && transfers.DELIVERED.length === 0 ? 
                <div className="flex rounded-lg shadow-sm p-4 mb-3 border-gray-200 justify-center font-light text-sm bg-gray-200">
                    No data found 
                </div> : 
                <>
                    {[...transfers.OUT, ...transfers.DELIVERED].map((request) => (
                        <div
                            key={request.id}
                            className="bg-white rounded-lg shadow-sm p-4 mb-3 border"
                        >
                            <div className="grid grid-cols-5 items-center text-sm font-light text-gray-800">
                                <div className="flex justify-center">{request.id}</div>
                                <div className="flex justify-center">{request.from_warehouse?.name}</div>
                                <div className="flex justify-center">{request.to_branch?.name || request.to_warehouse?.name}</div>
                                ...
                                <div className="flex justify-center">
                                    <Button
                                        className="!bg-yellow-500 text-white px-2 py-1 rounded-md text-sm hover:opacity-90"
                                        onClick={() => {
                                            handleViewing(request.id)
                                            setReviewOpen(true)
                                        }}
            ...
            <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
                <DialogContent className="max-w-lg">
                    <DialogTitle>
                        <ProcurementHeader label="Confirm Approval" />
                    </DialogTitle>
